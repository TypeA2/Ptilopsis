...
This section describes how the Ptilopsis compiler backend operates, what changes were made compared to Pareas, and what challenges were encountered. Whereas Pareas is written in Futhark, a language specifically designed for parallelism, we have chosen to implement Ptilopsis in \Cpp. This brings advantages in the ease of use of explicitly parallel AVX2 intrinsics, as well as the option to apply conventional programming methods to aid in implementing the compiler. We chose to implement Ptilopsis using AVX2 rather than AVX-512 because it still provides a feature set sufficient for implementing a parallel compiler, while being available on almost all relatively modern desktop, laptop and server CPUs at this time.
...
We observed that the location and cause of bottlenecks when compiling on a CPU differ from those observed when compiling on a GPU. It may be feasible to combine both of these approaches in a hybrid compiler, capable of offloading certain parts of the compilation on the GPU while running the rest on the CPU. This would require careful programming efforts regarding the offloading mechanism since, on most common systems, the link used to transfer data between CPU and GPU memory has relatively high latency compared to directly accessing local memory. In contrast, some GPU devices share their memory with the CPU, in which case this latency may not be present. Further research could be done into such an offloading mechanism and possible heuristics on when to use it.
